...
import os
import threading
from check_apis import large_deals_api_health, security_wise_archive_api_health, checking_internet_connection
...
import time;
from nse_large_deals import nse_largedeals
from nse_large_deals_custom import nse_largedeals_custom;
...
@synchronized
def main():
    if not checking_internet_connection():
        print("Internet Connection is not active please check")
        return;
    ...
    nse_securewise_api_health = security_wise_archive_api_health();
    nse_large_deals_api_health = large_deals_api_health();
    if not nse_securewise_api_health == "SUCCESS":
        logging.error(f"NSE Securewise API health check failed: {nse_securewise_api_health}");

    if selectedType == INPUT_TYPE_INDICES:
        if nse_securewise_api_health == "SUCCESS":
        ...
        if nse_large_deals_api_health == "SUCCESS":
            ...
            nse_largedeals(selectedDeals, fileName);
        ...
        fileName = getAutoGeneratedFileName(strTotal,reportsDir);
        logging.info("Started fetching large deals Data....");
        nse_largedeals_custom(selectedDeals, fileName,noOfDaysFromPresent);
        logging.info("Completed fetching large deals Data....");
        end_time = time.time();
        logging.info(f"Ended process time end_time = {end_time}");
        elapsed_time = end_time - start_time;
